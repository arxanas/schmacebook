...
$posts = "SELECT * FROM `posts` WHERE author_id=".$user->id;
foreach ($user->friends as $key => $value) {
	$posts .= " OR author_id=".$value;
}
$posts .= " ORDER BY post_id DESC LIMIT 0,20;";
	...
	for ($i=0; $i<mysql_num_rows($posts); $i++) {
		...
		if ($likes) {
			...
			foreach ($likes as $key => $value) {
				$liker_name = mysql_result(mysql_query("SELECT CONCAT(first_name, ' ', last_name) AS full_name FROM users WHERE id=".$value.";"), 0, 0);
				if ($key) {
					$like_html .= ", ";
				}
		...
		if ($dislikes) {
			...
			foreach ($dislikes as $key => $value) {
				$disliker_name = mysql_result(mysql_query("SELECT CONCAT(first_name, ' ', last_name) AS full_name FROM users WHERE id=".$value.";"), 0, 0);
				if ($key) {
					$dislike_html .= ", ";
				}
		...
		if (mysql_num_rows($comments)) {
			// weird for loop because it's hard to get the LAST three; first three is easy, of course
			for ($k=mysql_num_rows($comments)-1; $k>=0; $k--) {
				// because you can't mysql_fetch_assoc (because of the weird for loop), get comment properties manually
				...
				// if they were the one who commented, or the comment was on something they posted, or the comment was on something posted to them, or they're an admin
				...
				$comment_html .= "<span class='gray'>at ".$this_comment["time_posted"]."</span>";
		...
		$post_author = mysql_fetch_assoc(mysql_query("SELECT * FROM `users` WHERE id=".$this_post["author_id"].";"));
		$post_author_name = $post_author["first_name"]." ".$post_author["last_name"];
		$content .= "<tr><td>".$user->profile_picture_html($this_post["author_id"]);
		$content .= "<br /></td><td><strong><a href=\"wall.php?u=".$this_post["author_id"]."\">".$post_author_name."</a></strong> ";
		if ($this_post["to"] != 0) {
			$to_full_name = mysql_result(mysql_query("SELECT CONCAT(first_name, ' ' , last_name) FROM `users` WHERE id=".$this_post["to"].";"), 0, 0);
			$content .= "<img src=\"./images/to.png\" alt=\"\" /> <strong><a href=wall.php?u=".$this_post["to"].">".$to_full_name."</a></strong> ";
		}
		// if they wrote the post, or the post was to them, or they're an admin 
		if ($post_author["id"] == $user->id || $post_author["id"] == $this_post["to"] || $user->admin) {
			$content .= "<a href=\"javascript: delete_post(".$this_post["post_id"].");\" class='delete'></a>";
		}
		$content .= "<span class='gray'>at ".$this_post["time_posted"]."</span><br />";
		$content .= $this_post["content"]."<br />";
