...
function submit_status(id) {
	if (document.getElementById("status_txt").value) {
		...
		$.post("./ajax/php/send_wall_post.php", {
...
function submit_comment(id) {
	if (document.getElementById("comment_"+id).value) {
		...
		$.post("./ajax/php/send_comment.php", {
			...
			document.getElementById("comment_submit_btn_"+id).disabled = false;
...
function get_posts(id) {
	if (post_interval_disable_count == 0) {
		$.post("./ajax/php/get_wall_posts.php", {"to": user_id}, function(data, a, b) {
...
function show_comments (id) {
	...
	$.post("./ajax/php/get_comments.php", {"post_id": id}, function(data, a, b) {
...
function hide_comments (id) {
	post_interval_disable_count--;
	window.temp_comment_id = id;
	$.post("./ajax/php/get_comments.php", {"post_id": id, "preview": "true"}, function(data, a, b) {
		$("#comments_preview_"+window.temp_comment_id).html(data);
...
function edit_profile(id) {
	var profile_txt = $("#profile_"+id).html();
	$("#edit_profile").html("");
	$("#profile_"+id).html("<form action=\"wall.php\" method=\"post\"><textarea name=\"profile_txt\" id=\"profile_txt\" cols=\"35\" rows=\"1\" onkeyup=\"expandRows(this);\">"+profile_txt+"</textarea><br /><input type=\"submit\" class=\"btn\" value=\"Save Profile\" /></form>");
}